...
import CarSource from '../../data/cardb-source';
import { createListItemTemplate } from '../templates/template-creator';
import carList from '../templates/data.json'
...
`;
  	...
  	async afterRender() {
		const list = await CarSource.carlist();
		const listContainer = document.querySelector('#listContainer');
		list.forEach((carlist) => {
			listContainer.innerHTML += createListItemTemplate(carlist);
		});
	},
};
